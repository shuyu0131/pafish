...
- **评论闭环**：① **登录身份评论**——已登录用户自动以登录昵称/邮箱发表（服务端强制，不可伪造），前台隐藏昵称/邮箱输入；② **游客图形验证码**——未登录游客评论需输入 SVG 验证码（后台可关，点击图片可刷新），已登录用户免验证码；③ **被回复邮件通知**——评论者可在表单勾选"有人回复我时邮件通知"，被回复后系统向其邮箱发通知（需开启站点设置中的邮件通知并配置 SMTP）；④ **评论点赞**——游客可点赞，`liked_comments` cookie 记录（最多 200 个，一年有效）防刷，客户端乐观更新；⑤ **评论分页**——顶层评论每页 20 条，`?cpage=N` 深链，回复递归加载最多 5 层。
...
- **评论通知**：每次新评论/新回复生成站内通知（后台侧边栏铃铛徽标 + `/admin/notifications` 列表页，支持单条/全部已读）。邮件通知为可选：站点设置开启"邮件通知"并在后台「邮件服务 (SMTP)」卡片配置（或 `.env` 配置 `SMTP_HOST/PORT/USER/PASS/FROM` 兜底）后生效，发信失败静默不影响评论流程。

- **邮箱验证码闭环**：后台「邮件服务 (SMTP)」卡片可视化配置发信（host/port/user/pass/from，配置保存在服务器，含**发送测试邮件**按钮，未保存的表单值也能直接测试）。验证码覆盖两个场景：**注册**（`require_email_verify` 默认开启，验证码通过才创建账号，后台可关）与**忘记密码**（输邮箱 → 收 6 位验证码 → 输码+新密码直接重置；未注册邮箱统一返回成功防枚举）。验证码 10 分钟有效、一次性使用（用后即焚）、同邮箱 60 秒限频。
