...
import { installThemeFromUrl, installThemeFromZip } from "./actions";

// 主题安装：上传 zip 或输入下载地址，兼容商店分发 zip 格式
export function ThemeInstall({ canEdit }: { canEdit: boolean }) {
  ...
  async function installZip() {
    ...
    try {
      ...
      fd.append("zip", file);
      const r = await installThemeFromZip(fd);
      setMessage({ ok: true, text: `✓ 已安装主题 ${r.title} v${r.version}` });
  ...
  async function installByUrl() {
    if (!url.trim()) {
      setMessage({ ok: false, text: "请填写主题包下载地址" });
      return;
    }
    setPending("url");
    setMessage(null);
    try {
      const r = await installThemeFromUrl(url.trim());
      setMessage({ ok: true, text: `✓ 已安装主题 ${r.title} v${r.version}` });
      setUrl("");
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="card space-y-4 p-5">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium">安装主题</h2>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="https://example.com/themes/demo.zip"
              disabled={pending !== null}
            />
            <button
              type="button"
        ...
        <p className="text-xs text-muted">
          结构约定：zip 顶层目录为主题名（theme.json 声明 manifest 与设置项，可选 theme.css 覆盖配色）。
          安装前会校验目录名与路径安全，非法包将被拒绝。
        </p>
      </div>
    </div>
  );
